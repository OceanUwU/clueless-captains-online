...
const colorsAllowed = [...Array(100).keys()];
...
io.on('connection', socket => {
    ...
    socket.on('vote', num => {
        if (socket.ingame) {
            let match = matches[socket.ingame];
            if (match.allowVoting && (num === false || num === null || (Object.values(match.players).some(p => p.num === num && !p.dead) && !match.players[socket.id].dead)))
